...
def sshu_main():
    ...
    parser.add_argument('-o', "--output_folder", required=False, help="output folder, default: input folder_postprocessed")
    parser.add_argument('-l', nargs='+', type=int, required=True, help="List of label IDs (integer values)")
    ...
    if output_folder is None:
        output_folder = input_folder + '_postprocessed' if input_folder[-1] != os.sep else input_folder[:-1] + '_postprocessed'
    if isdir(output_folder):
        warnings.warn('folder {} exists, will be overwrite!'.format(output_folder))

    for_which_classes = [label_list, ]
